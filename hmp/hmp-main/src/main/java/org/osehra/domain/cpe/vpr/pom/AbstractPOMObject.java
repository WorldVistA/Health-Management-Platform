 ...
 * - no setters: more appropriate for healthcare, all inital data passed into constructor or via setData(Map)
 * - JPA + JSON serializatrion: designed to work for both relational and document oriented storage. 
 * - generic tag/flag mechanism that is indexed
 * 
 * See Patient for an example of a concrete class
 */
public abstract class AbstractPOMObject implements IPOMObject {
	// static initalization/configuration of a jackson mapper for all POM Objects to use
	...
	/*
	 * Unrecongnized fields are set this way.  This should not be public, all data setting 
	 * is done via setData()
	 */
	...
	{
		/*
		 * Implement in subclasses that are interested in loading UID-linked objects for things like DetailPanels.
		 */
	}
